...
if (
  (window.screen.availWidth <= 768 && window.innerHeight > window.innerWidth) ||
  "" == " horizontal"
) {
  restack(true);
}
...
function showFrameSize() {
  ...
  var width, height;
  width = Number(
    w3_getStyleValue(document.getElementById("iframeResult"), "width").replace(
      "px",
      ""
    )
  ).toFixed();
  height = Number(
    w3_getStyleValue(document.getElementById("iframeResult"), "height").replace(
      "px",
      ""
    )
  ).toFixed();
...
function fixDragBtn() {
  ...
  if (stack != " horizontal") {
    ...
    buttonwidth = Number(
      w3_getStyleValue(document.getElementById("dragbar"), "width").replace(
        "px",
        ""
      )
    );
    ...
    if (window.getComputedStyle) {
      textareawidth = window
        .getComputedStyle(document.getElementById("textareawrapper"), null)
...
var CLIENT_ID =
  "451843133508-ckbr5r6ch1ofqbmh87oll4u6ltinqv2t.apps.googleusercontent.com";
...
function onPickerApiLoad() {
  ...
  if (userAction == "save") {
    ...
    createFileWithHTMLContent(
      document.getElementById("googleFileName").value,
    ...
    if (httpB.readyState == 4 && httpB.status == 200) {
      if (httpB.responseText.substr(0, 2) == "OK") {
        ...
        document.getElementById("shareLink").href =
          "../code/tryit1b9e.html?filename=" + getId;
